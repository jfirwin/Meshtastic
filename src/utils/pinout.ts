...
export const MeshtasticLogoPath = (dimensions) => {
  ...
  let x = dimensions.xOffset - (logoWidth / 2) + (dimensions.logoScale * 10.049) + (dimensions.adjustedBoardWidth / 2)
...
export const BoardPath = (dimensions, left, right, top, bottom) => {
  ...
  const corners = {
    ...
    bottomRight: `C ${(dimensions.adjustedBoardWidth - corner.half) + dimensions.xOffset} ${dimensions.adjustedBoardHeight + dimensions.yOffset +0.5} ${dimensions.adjustedBoardWidth + dimensions.xOffset +0.5} ${(dimensions.adjustedBoardHeight - corner.half) + dimensions.yOffset} ${dimensions.adjustedBoardWidth + dimensions.xOffset} ${(dimensions.adjustedBoardHeight - corner.full) + dimensions.yOffset}`,
    topRight: `C ${dimensions.adjustedBoardWidth + dimensions.xOffset} ${corner.half + dimensions.yOffset -0.5} ${(dimensions.adjustedBoardWidth - corner.half) + dimensions.xOffset +0.5} ${dimensions.yOffset} ${(dimensions.adjustedBoardWidth - corner.full) + dimensions.xOffset} ${dimensions.yOffset}`
  }
  ...
  let path = [
    ...
    `L ${(dimensions.adjustedBoardWidth -  corner.full) + dimensions.xOffset} ${dimensions.adjustedBoardHeight + dimensions.yOffset}`, // bottom boardWidth
    `${corners.bottomRight}`, // bottom right corner
    `L ${dimensions.adjustedBoardWidth + dimensions.xOffset} ${corner.full + dimensions.yOffset}`, // right adjustedBoardHeight
    ...
    CirclePath(dimensions.adjustedBoardWidth - (dimensions.cutoutRadius * 2) + dimensions.xOffset, dimensions.yOffset + (dimensions.cutoutRadius * 2) ,dimensions.cutoutRadius), // top right corner cutout
    CirclePath(dimensions.adjustedBoardWidth - (dimensions.cutoutRadius * 2) + dimensions.xOffset, dimensions.adjustedBoardHeight - (dimensions.cutoutRadius * 2) + dimensions.yOffset ,dimensions.cutoutRadius), // bottom right corner cutout
  ...
  right.map((e, index) => {
    path.push(CirclePath(
      dimensions.adjustedBoardWidth - dimensions.silkscreenWidth + dimensions.xOffset, // x
      (dimensions.boardHeight / 2) + (dimensions.pinInterval / 2) + ((index) * (dimensions.pinInterval)) + dimensions.yOffset, // y
      dimensions.pinRadius // radius
    )) // Right side
  })
  top.map((e, index) => {
    path.push(CirclePath(
      (dimensions.adjustedBoardWidth / 2) + (dimensions.pinInterval / 2) + ((index) * (dimensions.pinInterval)) + dimensions.xOffset, // x
